...
- **DP1（价格来源，按发改委审计结论定稿 2026-07-07）**：无 NBS 分类指数提取件；发改委监测数据审计发现——种子价格完全缺失、柴油仅 2006–2010，但"农村居民服务价格"月报有**机耕费/机收费（元/亩）**2006–2010、2012、2014–2024（2011与2013断档；"2013"命名文件实为2012年数据），肥料/农膜/农药 2003–2025 连续高质量。据此：
  - labor = 劳动日工价（汇编直接给出，单位值）。
  - fert = 化肥费 ÷ 化肥折纯量（汇编单位值，元/公斤纯养分）；用发改委尿素省级年均价交叉校验。
  - seed = 种子费 ÷ 每亩种子用量（汇编单位值；发改委无种子价）。质量混杂（品种升级）在正文注明，稳健性用发改委投入品综合指数替代。
  - mach = 发改委机耕费+机收费省级年均价（元/亩，正是机械服务的市场租金概念）；2011与2013断档按省对数线性插值；2004–05 用汇编机械作业费省级亩均值增长率外推，全部记录日志。
  - other = 发改委农药（毒死蜱，全期一致口径）与地膜价格按汇编份额 Törnqvist 合成。
...
来源 `/root/data/数据/发改委价格数据/`（审计报告 `docs/data_audit_ndrc.md`，清洗脚本 `python/build_prices_ndrc.py`）：
- 农业生产资料价格 2003.04–2025（多源拼接：真xls + GB18030伪xls + HTML伪xls + 错位CSV修复）→ 省×年：尿素/碳铵/过钙/二铵/复合肥、地膜/棚膜、毒死蜱；柴油/氯化钾/仔猪/豆粕/麦麸仅 2006–2010。
- 农村居民服务价格月报 → 机耕费/机收费（元/亩）2006–2010、2012、2014–2024（2011与2013断档插值）→ w_mach。
- 聚合规则：旬（周/月）×县内采价点中位数 → 省内县中位数 → 年度平均；规格合并按审计§2规则（二铵两代命名合并；16%过钙、硫基复合肥不与基准规格混合）；log价格 MAD |z|>5 剔除；全流程写 `price_construction_log.csv`。
