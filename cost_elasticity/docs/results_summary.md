...
## 4. 诱致性创新两阶段（S2 general index，pooled 5 大粮作）——【修正：不支持Hicks】

节约偏向定义 sb_n=−Δδ_n（δ为份额方程年哑变量的累计技术移动）。回归结果
labor Σψ=−0.236 (t=−5.2)、mach Σψ=−0.109 (t=−4.7)：**Σψ<0 表示滞后工资相对
上涨之后出现的是劳动使用（δ_labor 上移）方向的移动，与 Hicks 诱致性预期
（Σψ>0）方向相反**（此前版本误读为"方向符合"，已更正）。结合 labor 的
placebo（未来一期价格 t=−3.8，量级达主效应 69%）显著，一致的解释是：
年哑变量吸收了 γ 未完全捕捉的全国性价格-份额调整（工资涨→劳动份额机械上升），
第二阶段识别的是这一残余价格效应/共同趋势，而非诱致性技术响应。
**该节结果目前不能作为诱致性创新的证据**；待做修补（评审建议）：年份FE、
省×品种层面第二阶段、更长滞后（k=1–5）、去趋势——若仍不奏效则从贡献中移除。
