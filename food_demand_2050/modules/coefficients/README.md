...
| `coefficient_boundary_audit.csv` | Row-level boundary and double-counting audit generated by `audit_boundaries.py` | metadata | This repository |
...
For revision control, run `python3 modules/coefficients/audit_boundaries.py` after any coefficient edit. The generated audit table records the indicator, commodity, region, boundary, direct-vs-lifecycle classification, feed/LUC/processing/transport flags, imputation flag, and whether the row can be summed with other commodity coefficients in the direct production account.
