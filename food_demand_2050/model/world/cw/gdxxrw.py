...
def parse_ref(ref):
    ...
    cell = cell.strip()
    if cell == "":          # bare 'sheet!' anchor -> top-left cell A1
        return sheet.strip().strip("'"), 1, 1
...
class Workbook:
    def __init__(self, path):
        # legacy .xls (OLE2/BIFF8) — openpyxl cannot read these; use the
        # bundled minimal BIFF reader, which yields Sheet-compatible objects.
        if str(path).lower().endswith(".xls"):
            from cw.xls_reader import read_xls
            self._biff = read_xls(str(path))
            self._sheets = {}
            self._titlemap = {n.strip().upper(): n for n in self._biff}
            self._is_biff = True
            return
    ...
    def sheet(self, name):
        ...
        if key not in self._sheets:
            if getattr(self, "_is_biff", False):
                self._sheets[key] = self._biff[self._titlemap[key]]
            else:
