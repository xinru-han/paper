...
from .data import Data, T, TAH, TC, TSP, TPREV
...
class Output:
    def __init__(self, casm_dir, d: Data, m, res, sim="BASE"):
        self.d, self.m = d, m
        self.sim = sim
        C, H = d.C, d.H
        n = len(C)
        ...
        # emission-factor sheets carry one column block per SIM (base,
        # SIM1..SIM5); 4OUTPUT.gms uses FERTEF0(C,SIM,T) with the run's own
        # SIM label, so pick this scenario's block (unassigned SIMs -> 0,
        # exactly like GAMS)
        def par_ct(sym):
            df = pd.DataFrame(0.0, index=C, columns=T)
            for (c, s, t), v in par[sym].items():
                if c in df.index and s == sim.upper() and t in df.columns:
                    df.loc[c, t] = v
            return df
        ...
        # 4OUTPUT.gms anchors the trade adjustment at literal year '2023',
        # which is TPREV in the paper version (it was TPHIS in v2.2.8)
        NETQMEXADJ = NETQMEX.sub(NETQMEX[TPREV], axis=0)
