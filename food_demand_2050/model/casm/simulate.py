...
class Growth:
    ...
    def __init__(self, casm_dir, sim="BASE", overrides=None):
        # the paper workbooks anchor their Index sheet at A1 (header row 1,
        # entries from row 2); `wanted` filters out the extra symbols
        raw = load_symbols(os.path.join(casm_dir, "2simulation.xlsm"),
                           "INDEX!A1", wanted=set(_GR_SYMBOLS))
        self.raw = raw
        self.sim = sim
...
def run_base(casm_dir, progress=None, growth_overrides=None, sim="BASE",
             prebuilt=None):
    """Solve one scenario recursively over TC.

    sim       -- SIM column of 2simulation.xlsm to use ('BASE', 'SIM1', ...).
                 SIMn.gms runs the identical loop with SIMC=SIMn, so scenario
                 runs only differ in which growth-rate column is read.
    prebuilt  -- optional (d, cal, m) triple to reuse across scenarios of the
                 same folder (0data/1calibration do not depend on the SIM).
    """
    if prebuilt is None:
        d = Data(casm_dir)
        cal = Calibration(d)
        ...
        d, cal, m = prebuilt
    gr = Growth(casm_dir, sim=sim, overrides=growth_overrides)
    C, H, n = d.C, d.H, m.n
