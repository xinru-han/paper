...
forvalues g = 1/6 {
    ...
    * symmetrically without imposing a category-specific market-price cap.
    foreach kind in rep range {
        ...
        bysort data_year: egen double med_`kind'`g' = median(ln_`kind'`g')
        gen double ad_`kind'`g' = abs(ln_`kind'`g' - med_`kind'`g')
        bysort data_year: egen double mad_`kind'`g' = median(ad_`kind'`g')
    ...
    * from creating a spurious price-level shift.
    ...
    replace p`g'_direct = range`g'_cal if missing(p`g'_direct) & range`g'_cal > 0
    replace p`g'_direct_method = 2 if missing(p`g'_direct_method) & !missing(p`g'_direct)

    * A second screen is necessary after the calibrated and representative
    * series are combined because their pooled tail can differ from either
    * component's tail. Screen before constructing every geographic donor.
    gen double ln_direct`g' = ln(p`g'_direct) if p`g'_direct > 0
    bysort data_year: egen double med_direct`g' = median(ln_direct`g')
    gen double ad_direct`g' = abs(ln_direct`g' - med_direct`g')
    bysort data_year: egen double mad_direct`g' = median(ad_direct`g')
    ...
    quietly count if p`g'_direct_method == 2
    post `amem' (`g') ("calibrated_midrange_villages") (r(N))
...
preserve
    ...
    export delimited using "$EASI_OUT/price_quote_audit.csv", replace
restore

keep village_id data_year town_id county_id province_id vilLat vilLon p*_direct p*_direct_method
...
label define price_source 1 "own village: representative products" ///
    2 "own village: calibrated category midpoint" 3 "same-town direct median" ///
