# -*- coding: utf-8 -*-
"""按 simulation_plan_v3.csv 运行CASM（Python版）机械化情景模拟。
...
输出（results/casm/）：
  casm_results_long_v3.csv     基线+9情景 × 品种 × 年份 明细
  table7_grain_v3.csv          粮食总量：产量/净贸易/自给率/面积/单产（2030）
  table8_cereal_staple_v3.csv  谷物与口粮安全（2030）
"""
import os
import sys
import time
...
plan = pd.read_csv(os.path.join(OUT, "simulation_plan_v3.csv"),
                   encoding="utf-8-sig")
plan = plan[plan["情景"].str.startswith("S")].copy()
...
pd.DataFrame(long_rows).to_csv(os.path.join(OUT, "casm_results_long_v3.csv"),
...
t7.to_csv(os.path.join(OUT, "table7_grain_v3.csv"), index=False,
          encoding="utf-8-sig")
print("\n== 表7 粮食总量（2030）==")
...
t8.to_csv(os.path.join(OUT, "table8_cereal_staple_v3.csv"), index=False,
          encoding="utf-8-sig")
print("\n== 表8 谷物与口粮安全（2030）==")
