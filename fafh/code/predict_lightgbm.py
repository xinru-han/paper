#!/usr/bin/env python3
"""
使用 LightGBM 预测户外消费系数。
包含：1.Copula收入分布匹配 2.Kriging空间插值 3.Bootstrap预测区间(见run_bootstrap.py)
4.早停(verbosity) 5.超参数固定 6.统一随机种子(42) 7.补全阶段交叉验证。
"""
...
BASE_DIR = os.getcwd()
OUTPUT_FILE = os.path.join(BASE_DIR, "predictions_lightgbm.csv")
OUTPUT_FILE_ROBUST = os.path.join(BASE_DIR, "predictions_lightgbm_robust.csv")
OUTPUT_FILE_BOOTSTRAP = os.path.join(BASE_DIR, "predictions_lightgbm_bootstrap.csv")
...
def train_lightgbm_model(X_train, y_train, X_val=None, y_val=None):
    model = lgb.LGBMRegressor(n_estimators=100, max_depth=6, learning_rate=0.1, random_state=SEED, verbosity=-1)
...
def predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_model_func):
    ...
    if all_results:
        ...
        df_final.to_csv(os.path.join(BASE_DIR, "grain_structure_predictions_lightgbm.csv"), index=False)
        return df_final
    return None
...
def predict_category(q_col, total_col, ratio_col, df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, use_imputation=False):
    ...
    if df_clean is None and use_imputation:
        ...
        if total_col in df_micro.columns and home_col in df_micro.columns:
            df_filled = impute_home_total_and_ratio(df_micro, feature_cols, total_col, home_col, train_lightgbm_model)
    ...
    model = train_lightgbm_model(X_train, y_ratio)
    X_pred_processed, _, _ = prepare_features_combined(X_pred_feats, feature_cols, scaler, label_enc)
...
if __name__ == "__main__":
    ...
    predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_lightgbm_model)
        ...
        if all_results:
            ...
            if seed_env == "42":
                ...
                if out_path == OUTPUT_FILE_ROBUST:
                    grain_robust = os.path.join(BASE_DIR, "grain_structure_predictions_lightgbm_robust.csv")
                    if os.path.isfile(os.path.join(BASE_DIR, "grain_structure_predictions_lightgbm.csv")):
                        import shutil
                        shutil.copy(os.path.join(BASE_DIR, "grain_structure_predictions_lightgbm.csv"), grain_robust)
