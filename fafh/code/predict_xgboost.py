...
BASE_DIR = os.getcwd()
OUTPUT_FILE = os.path.join(BASE_DIR, "predictions_xgboost.csv")
OUTPUT_FILE_ROBUST = os.path.join(BASE_DIR, "predictions_xgboost_robust.csv")
OUTPUT_FILE_BOOTSTRAP = os.path.join(BASE_DIR, "predictions_xgboost_bootstrap.csv")
...
def train_xgboost_model(X_train, y_train, X_val=None, y_val=None):
    model = xgb.XGBRegressor(objective='reg:squarederror', max_depth=6, learning_rate=0.1,
...
def predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_model_func):
    ...
    df_micro['Total_Grain_Mass'] = (
        df_micro['calc_稻谷'].fillna(0) + df_micro['total_小麦'].fillna(0) +
        df_micro['total_豆类'].fillna(0) + total_za
    )
    ...
    for name, (total_col_name, grain_type) in targets.items():
        if total_col_name not in df_grain.columns:
            continue
        y_series = (df_grain[total_col_name] / df_grain['Total_Grain_Mass']).clip(0, 1)
        df_train = df_grain[feature_cols].copy()
        df_train['target_y'] = y_series
        ...
        for train_idx, val_idx in tscv.split(X_train):
            model = train_model_func(X_train[train_idx], y_target[train_idx])
            pred = model.predict(X_train[val_idx])
            scores.append(np.mean(np.abs(pred - y_target[val_idx])))
    ...
    if all_results:
        ...
        df_final.to_csv(os.path.join(BASE_DIR, "grain_structure_predictions_xgboost.csv"), index=False)
        return df_final
    return None
...
def predict_category(q_col, total_col, ratio_col, df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, use_imputation=False):
    ...
    if df_clean is None and use_imputation:
        ...
        if total_col in df_micro.columns and home_col in df_micro.columns:
            df_filled = impute_home_total_and_ratio(df_micro, feature_cols, total_col, home_col, train_xgboost_model)
    ...
    scores = []
    for train_idx, val_idx in tscv.split(X_train):
        model = train_xgboost_model(X_train[train_idx], y_ratio[train_idx])
        pred = model.predict(X_train[val_idx])
        scores.append(np.mean(np.abs(pred - y_ratio[val_idx])))
    model = train_xgboost_model(X_train, y_ratio)
    X_pred_processed, _, _ = prepare_features_combined(X_pred_feats, feature_cols, scaler, label_enc)
...
if __name__ == "__main__":
    ...
    predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_xgboost_model)
    ...
    for use_impute, out_path, label in [(True, OUTPUT_FILE, "主流程（补缺失）"), (False, OUTPUT_FILE_ROBUST, "稳健性（不补缺失）")]:
        ...
        if all_results:
            ...
            df_pivot = df_final.pivot_table(index=["Province", "Year"], columns="Category", values="Coef_Final", aggfunc="first").reset_index()
            ...
            if seed_env == "42":
                ...
                if out_path == OUTPUT_FILE_ROBUST:
                    import shutil
                    g = os.path.join(BASE_DIR, "grain_structure_predictions_xgboost.csv")
                    if os.path.isfile(g):
                        shutil.copy(g, os.path.join(BASE_DIR, "grain_structure_predictions_xgboost_robust.csv"))
