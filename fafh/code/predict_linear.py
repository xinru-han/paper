...
BASE_DIR = os.getcwd()
OUTPUT_FILE = os.path.join(BASE_DIR, "predictions_linear.csv")
OUTPUT_FILE_ROBUST = os.path.join(BASE_DIR, "predictions_linear_robust.csv")
OUTPUT_FILE_BOOTSTRAP = os.path.join(BASE_DIR, "predictions_linear_bootstrap.csv")
...
def predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_model_func):
    ...
    if all_results:
        ...
        df_final.to_csv(os.path.join(BASE_DIR, "grain_structure_predictions_linear.csv"), index=False)
        return df_final
    return None
...
def predict_category(q_col, total_col, ratio_col, df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, use_imputation=False):
    ...
    if df_clean is None and use_imputation:
        ...
        if total_col in df_micro.columns and home_col in df_micro.columns:
            df_filled = impute_home_total_and_ratio(df_micro, feature_cols, total_col, home_col, train_linear_model)
    ...
    model = train_linear_model(X_train, y_ratio)
    X_pred_processed, _, _ = prepare_features_combined(X_pred_feats, feature_cols, scaler, label_enc)
...
if __name__ == "__main__":
    print("当前使用: Ridge 线性回归 预测户外消费系数")
    ...
    predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_linear_model)
        ...
        if all_results:
            ...
            if seed_env == "42":
                ...
                if out_path == OUTPUT_FILE_ROBUST:
                    import shutil
                    g = os.path.join(BASE_DIR, "grain_structure_predictions_linear.csv")
                    if os.path.isfile(g):
                        shutil.copy(g, os.path.join(BASE_DIR, "grain_structure_predictions_linear_robust.csv"))
