...
BASE_DIR = os.getcwd()
OUTPUT_FILE = os.path.join(BASE_DIR, "predictions_catboost.csv")
OUTPUT_FILE_ROBUST = os.path.join(BASE_DIR, "predictions_catboost_robust.csv")
OUTPUT_FILE_BOOTSTRAP = os.path.join(BASE_DIR, "predictions_catboost_bootstrap.csv")
...
def train_catboost_model(X_train, y_train, X_val=None, y_val=None):
    model = CatBoostRegressor(iterations=100, depth=6, learning_rate=0.1, random_seed=SEED, verbose=0)
...
def predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_model_func):
    ...
    for name, (total_col_name, grain_type) in targets.items():
        ...
        for train_idx, val_idx in tscv.split(X_train):
            ...
            scores.append(np.mean(np.abs(model.predict(X_train[val_idx]) - y_target[val_idx])))
    ...
    if all_results:
        ...
        df_final.to_csv(os.path.join(BASE_DIR, "grain_structure_predictions_catboost.csv"), index=False)
        return df_final
    return None
...
def predict_category(q_col, total_col, ratio_col, df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, use_imputation=False):
    ...
    if df_clean is None and use_imputation:
        ...
        if total_col in df_micro.columns and home_col in df_micro.columns:
            df_filled = impute_home_total_and_ratio(df_micro, feature_cols, total_col, home_col, train_catboost_model)
    ...
    model = train_catboost_model(X_train, y_ratio)
    X_pred_processed, _, _ = prepare_features_combined(X_pred_feats, feature_cols, scaler, label_enc)
...
if __name__ == "__main__":
    ...
    predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_catboost_model)
        ...
        if all_results:
            ...
            if seed_env == "42":
                ...
                if out_path == OUTPUT_FILE_ROBUST:
                    import shutil
                    g = os.path.join(BASE_DIR, "grain_structure_predictions_catboost.csv")
                    if os.path.isfile(g):
                        shutil.copy(g, os.path.join(BASE_DIR, "grain_structure_predictions_catboost_robust.csv"))
