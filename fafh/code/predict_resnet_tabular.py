...
BASE_DIR = os.getcwd()
OUTPUT_FILE = os.path.join(BASE_DIR, "predictions_resnet_tabular.csv")
OUTPUT_FILE_ROBUST = os.path.join(BASE_DIR, "predictions_resnet_tabular_robust.csv")
OUTPUT_FILE_BOOTSTRAP = os.path.join(BASE_DIR, "predictions_resnet_tabular_bootstrap.csv")
IMPUTED_RATIOS_FILE = os.path.join(BASE_DIR, "data", "imputed_ratios_best.csv")
SEED = int(os.environ.get("PREDICT_SEED", 42))
MODEL_NAME = "resnet_tabular"
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


class _ResNetPredictWrapper:
    """Wrapper so ResNet can be used with impute_home_total_and_ratio (expects .predict(X))."""
    def __init__(self, model):
        self.model = model
    def predict(self, X):
        with torch.no_grad():
            return self.model(torch.FloatTensor(X).to(device)).cpu().numpy()
...
class ResNetBlock(nn.Module):
    ...
    def forward(self, x):
        return x + torch.relu(self.bn(self.fc(x)))

class ResNetRegressor(nn.Module):
    def __init__(self, input_dim, hidden=64, num_blocks=2):
        super().__init__()
        self.fc_in = nn.Linear(input_dim, hidden)
        self.blocks = nn.Sequential(*[ResNetBlock(hidden) for _ in range(num_blocks)])
...
def train_resnet_model(X_train, y_train, X_val=None, y_val=None):
    X = torch.FloatTensor(X_train).to(device)
    y = torch.FloatTensor(y_train).to(device)
    model = ResNetRegressor(X_train.shape[1]).to(device)
    ...
    for _ in range(100):
        ...
        loss.backward()
        opt.step()
    return _ResNetPredictWrapper(model)
...
def predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_model_func):
    ...
    if all_results:
        ...
        df_final.to_csv(os.path.join(BASE_DIR, "grain_structure_predictions_resnet_tabular.csv"), index=False)
        return df_final
    return None
...
def predict_category(q_col, total_col, ratio_col, df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, use_imputation=False):
    ...
    if df_clean is None and use_imputation:
        ...
        if total_col in df_micro.columns and home_col in df_micro.columns:
            df_filled = impute_home_total_and_ratio(df_micro, feature_cols, total_col, home_col, train_resnet_model)
    ...
    model = train_resnet_model(X_train, y_ratio)
    X_pred_processed, _, _ = prepare_features_combined(X_pred_feats, feature_cols, scaler, label_enc)
    s_pred = model.predict(X_pred_processed)
    s_pred = np.clip(s_pred, 1e-6, 1.0)
...
if __name__ == "__main__":
    print("当前使用: ResNet (Tabular) 预测户外消费系数")
    ...
    predict_grain_structure(df_micro, X_pred_feats, feature_cols, known_provinces, X_pred, train_resnet_model)
        ...
        if all_results:
            ...
            if seed_env == "42":
                ...
                if out_path == OUTPUT_FILE_ROBUST:
                    import shutil
                    g = os.path.join(BASE_DIR, "grain_structure_predictions_resnet_tabular.csv")
                    if os.path.isfile(g):
                        shutil.copy(g, os.path.join(BASE_DIR, "grain_structure_predictions_resnet_tabular_robust.csv"))
