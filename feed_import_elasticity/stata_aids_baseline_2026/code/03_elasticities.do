...
log using `"`logs'/03_elasticities.log"', text replace

use `"`project'/data/aids_estimation_panel.dta"', clear
...
tempname mem table
tempfile results
postfile `mem' str12 elasticity_type byte demand_good shock_good ///
...
forvalues i = 1/5 {
    ...
    quietly nlcom (elasticity: 1 + (`b`i'') / (`w`i''))
    matrix `table' = r(table)
    post `mem' ("expenditure") (`i') (0) (`share') (`table'[1,1]) (`table'[2,1]) ///
        (`table'[3,1]) (`table'[4,1]) (`table'[5,1]) (`table'[6,1]) ("`stars'") ///
        (`punit') ("`unitstars'")
}
...
forvalues i = 1/5 {
    ...
    forvalues j = 1/5 {
        ...
        foreach type in marshallian hicksian {
            ...
            local pv = `table'[4,1]
            local stars = cond(`pv' < .01, "***", cond(`pv' < .05, "**", cond(`pv' < .10, "*", "")))
            post `mem' ("`type'") (`i') (`j') (`share') (`table'[1,1]) (`table'[2,1]) ///
                (`table'[3,1]) (`table'[4,1]) (`table'[5,1]) (`table'[6,1]) ("`stars'") ///
                (.) ("")
        }
    }
...
order elasticity_type demand_product price_product reference_share estimate std_error z_value p_value ci_low ci_high significance
sort elasticity_type demand_good shock_good
export delimited using `"`output'/basic_aids_elasticities_complete.csv"', replace
save `"`output'/basic_aids_elasticities_complete.dta"', replace
