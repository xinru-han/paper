...
def nutrition_lookup() -> dict[str, float]:
    ...
    grain_weights.to_csv(OUT / "original_item_grain_weights_used.csv", index=False)
    nutrition.to_csv(OUT / "original_item_nutrition_used.csv", index=False)
    return lookup


def build_original_item_panel(variant: str) -> pd.DataFrame:
    spec = VARIANTS[variant]
    base = pd.read_csv(spec["base_panel"])
    raw = pd.read_stata(ROOT / "ProvinceData" / "workdata" / "data.dta")
    keep = ["province", "year"] + sorted(
    ...
    ].merge(raw, on=["province", "year"], how="left")
    ...
    for item in ITEM_ORDER:
        if item == "nonfood":
            continue
        item_spec = pipe.FOOD_ITEMS[item]
        ...
        data[f"x_{item}"] = q * kcal_kg / 365.0 / 2000.0
        data[f"p_{item}_model"] = p * q / data[f"x_{item}"]
        data[f"e_{item}"] = p * q
        data[f"{item}_kg_per_cap_year"] = q
        data[f"{item}_kcal_per_kg"] = kcal_kg
        food_exp_cols.append(f"e_{item}")
        food_x_cols.append(f"x_{item}")

    data["covered_food_exp_original_items"] = data[food_exp_cols].sum(axis=1)
    data["nonfood_exp_original_items"] = data["m"] - data["covered_food_exp_original_items"]
    data["x_nonfood"] = data["nonfood_exp_original_items"] / data[spec["nonfood_price_column"]]
    data["p_nonfood_model"] = data[spec["nonfood_price_column"]]
    data["covered_daily_kcal_original_items"] = data[food_x_cols].sum(axis=1) * 2000
    ...
    cols = [
        ...
        "m",
        "food_price_index_2023",
        "covered_food_exp_original_items",
        "nonfood_exp_original_items",
        "covered_daily_kcal_original_items",
    ]
    for item in ITEM_ORDER:
        cols += [f"x_{item}", f"p_{item}_model"]
        ...
        invalid.append(
            {
                "variant": variant,
                "item": item,
                "n_missing_x": int(panel[f"x_{item}"].isna().sum()),
                "n_nonpositive_x": int((panel[f"x_{item}"] <= 0).sum()),
                "n_missing_p": int(panel[f"p_{item}_model"].isna().sum()),
                "n_nonpositive_p": int((panel[f"p_{item}_model"] <= 0).sum()),
                "x_min": float(panel[f"x_{item}"].min()),
                "x_median": float(panel[f"x_{item}"].median()),
                "x_max": float(panel[f"x_{item}"].max()),
                "p_min": float(panel[f"p_{item}_model"].min()),
                "p_median": float(panel[f"p_{item}_model"].median()),
                "p_max": float(panel[f"p_{item}_model"].max()),
            }
        )
    pd.DataFrame(invalid).to_csv(OUT / f"{variant}__data_diagnostics.csv", index=False)
    bad_nonfood = int((panel["nonfood_exp_original_items"] <= 0).sum())
    if bad_nonfood:
        raise RuntimeError(f"{variant}: nonfood residual is non-positive for {bad_nonfood} observations.")
    if panel[[f"x_{i}" for i in ITEM_ORDER] + [f"p_{i}_model" for i in ITEM_ORDER]].isna().any().any():
        raise RuntimeError(f"{variant}: missing model x or p values.")
    if (panel[[f"x_{i}" for i in ITEM_ORDER] + [f"p_{i}_model" for i in ITEM_ORDER]] <= 0).any().any():
        raise RuntimeError(f"{variant}: non-positive model x or p values.")
    return panel


def panel_to_original_arrays(panel: pd.DataFrame) -> pipe.ModelArrays:
    return pipe.ModelArrays(
        ...
        group_names=ITEM_ORDER,
        x=panel[[f"x_{g}" for g in ITEM_ORDER]].to_numpy(float),
        p=panel[[f"p_{g}_model" for g in ITEM_ORDER]].to_numpy(float),
...
def elasticity_for_point(
    ...
    x_minus, _ = pred_at_income(max(m - step, 1e-6))
    x_plus, _ = pred_at_income(m + step)
    eta = (np.log(x_plus) - np.log(x_minus)) / (np.log(m + step) - np.log(max(m - step, 1e-6)))
            ...
            param_rows.append(
                {
                    ...
                    "model": fit["model"],
                    "group": group,
                    "group_label_cn": ITEM_LABEL_CN[group],
...
def model_comparison(variant: str, fits: tuple[dict, dict], fit_by_group: pd.DataFrame, n_obs: int) -> pd.DataFrame:
    rows = []
    k = {"AIDADS_sat": 2 * len(ITEM_ORDER) + 1, "MAIDADS_sat": 3 * len(ITEM_ORDER) + 2}
    for fit in fits:
        rows.append(
            {
                "variant": variant,
...
def elasticity_outputs(variant: str, panel: pd.DataFrame, params: dict[str, np.ndarray | float]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    group_names = ITEM_ORDER
    p_mean = panel[panel["year"].eq(2023)][[f"p_{g}_model" for g in group_names]].mean().to_numpy(float)
    incomes = np.array(
        sorted(
            set(
    ...
    for income in incomes:
        try:
            mar, hic, eta, budget_shares, u = price_elasticities_for_point(p_mean, float(income), params, group_names)
            _, xhat, _, phi = elasticity_for_point(p_mean, float(income), params, group_names)
        ...
        for i, group in enumerate(group_names):
            ...
            for j, pgroup in enumerate(group_names):
                base = {
                    "variant": variant,
                    "income": income,
                    "demand_group": group,
                    "demand_group_label_cn": ITEM_LABEL_CN[group],
                    "price_group": pgroup,
                    "price_group_label_cn": ITEM_LABEL_CN[pgroup],
...
def anomaly_report(
    ...
    lines.append("")
    lines.append("### 分品类拟合误差")
    lines.append("")
    lines.append(md_table(fit_by_group.sort_values(["model", "relative_rmse"], ascending=[True, False])))
    lines.append("")
    lines.append("### 异常检查")
    lines.append("")
    issues = []
    ...
    if issues:
        ...
        lines.append("- 未发现明显数值异常。")
    lines.append("")
    lines.append("### 弹性一致性最大误差")
    lines.append("")
    if not consistency.empty:
        summary = consistency[cons_cols].abs().max().reset_index()
        summary.columns = ["check", "max_abs_error"]
        lines.append(md_table(summary, digits=6))
    else:
        lines.append("_未生成一致性检查。_")
    lines.append("")
    return lines


def run_variant(variant: str, maxiter_a: int, maxiter_m: int, random_scales: tuple[float, ...], seed: int) -> dict:
    print(f"Building original-item panel for {variant}...", flush=True)
    panel = build_original_item_panel(variant)
    panel.to_csv(OUT / f"{variant}__original_item_panel.csv", index=False)
    arr = panel_to_original_arrays(panel)
    print(f"Fitting original-item AIDADS/MAIDADS for {variant}...", flush=True)
    fits = pipe.fit_model(
        arr,
        maidads_random_scales=random_scales,
        maxiter_a=maxiter_a,
        maxiter_m=maxiter_m,
        progress=True,
        seed=seed,
    ...
    main_params = fits[1]["params"]
    income_elasticity, price_m, price_h, consistency = elasticity_outputs(variant, panel, main_params)
...
def write_summary(outputs: list[dict], started_at: str) -> None:
    lines = [
        ...
        "## 原始品类与模型品类",
        "",
        md_table(pd.DataFrame({"group": ITEM_ORDER, "label_cn": [ITEM_LABEL_CN[g] for g in ITEM_ORDER]})),
        "",
    ]
    for out in outputs:
        lines.extend(
    ...
    (OUT / "ORIGINAL_ITEM_RESULTS.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("--maxiter-a", type=int, default=650)
    parser.add_argument("--maxiter-m", type=int, default=900)
    ...
    (OUT / "original_item_run_manifest.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    print(OUT / "ORIGINAL_ITEM_RESULTS.md")
