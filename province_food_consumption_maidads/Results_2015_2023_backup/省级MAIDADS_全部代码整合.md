...
- 生成时间：2026-06-09 23:26:36
- 工作目录：`/Users/hanxinru/Desktop/省级食物消费`

## 一、运行顺序

```bash
cd /Users/hanxinru/Desktop/省级食物消费
...
| 文件 | 行数 | 作用 |
| --- | ---: | --- |
| `run_maidads_pipeline.py` | 1818 | 数据构造、MAIDADS/AIDADS 主估计、弹性、预测和主摘要生成 |
| `run_additional_checks.py` | 620 | CPI 稳健性、样本外验证、bootstrap 和追加摘要生成 |
| `run_formal_bootstrap.py` | 442 | 正式规模省份簇 bootstrap 与 LR cluster bootstrap，可断点续跑并同步正式推断结果 |
...
源文件：`/Users/hanxinru/Desktop/省级食物消费/ProvinceMAIDADS/scripts/run_maidads_pipeline.py`
...
GROUPS = {
    ...
    "meatsea": ["pork", "beef", "mutton", "poultry", "aquatic"],
    "dairyegg": ["milk", "egg"],
    "nonfood": [],
}
...
def fit_model(
    ...
    res_a = minimize(
        ...
        options={"maxiter": maxiter_a, "ftol": 1e-8, "maxls": 30},
    )
    if progress:
        print(
    ...
    for scale in maidads_random_scales:
    ...
    for i, start in enumerate(starts, start=1):
        ...
        res = minimize(
            ...
            options={"maxiter": maxiter_m, "ftol": 1e-8, "maxls": 30},
        )
        if progress:
            print(
...
def build_results(panel: pd.DataFrame, arr: ModelArrays, fits: tuple[dict, dict], nutrition: pd.DataFrame) -> dict:
    ...
    for m in income_grid:
        ...
        animal_idx = [group_names.index("meatsea"), group_names.index("dairyegg")]
...
源文件：`/Users/hanxinru/Desktop/省级食物消费/ProvinceMAIDADS/scripts/run_additional_checks.py`
...
源文件：`/Users/hanxinru/Desktop/省级食物消费/ProvinceMAIDADS/scripts/run_formal_bootstrap.py`
...
def _run_pool(kind: str, draws: list[int], workers: int, task_fn, append_fn, progress_every: int) -> None:
    ...
    completed = 0
    started = time.time()
    with futures.ProcessPoolExecutor(max_workers=workers, initializer=_init_worker) as ex:
...
源文件：`/Users/hanxinru/Desktop/省级食物消费/ProvinceMAIDADS/scripts/prepare_paper_workflow_outputs.py`
...
源文件：`/Users/hanxinru/Desktop/省级食物消费/ProvinceMAIDADS/scripts/build_manuscript_draft.py`
...
源文件：`/Users/hanxinru/Desktop/省级食物消费/ProvinceMAIDADS/scripts/build_maidads_simulator_workbook.py`
...
源文件：`/Users/hanxinru/Desktop/省级食物消费/ProvinceMAIDADS/scripts/compile_markdown_outputs.py`
