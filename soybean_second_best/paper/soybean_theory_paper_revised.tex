...
校准后的模型在两个层面复现观测。规划模型解得社会最优规模 $Y^*=2075$ 万吨，与 2025 年观测产量 2090.5 万吨偏差 $-0.7\%$；比较静态符号全部与命题 1--2 一致（$\partial Y^*/\partial\rho>0$、$\partial Y^*/\partial\bar p>0$、$\partial Y^*/\partial\beta>0$、$\partial Y^*/\partial\omega<0$）。农户模型在基线政策下 2026--2035 年窗口均值产量为 2046 万吨（相对观测偏差 $-2.1\%$，随机稳态经模拟矩方法校正至观测点），并复现观测的区域面积份额（$L^1$ 偏差 1.2\%）。

六项命题（H1--H6）的结构检验中，前五项全部通过，第六项部分通过。H1（风险—产能）：进口风险倍增使 $Y^*$ 增加 450 万吨，而在高成本曲率下仅增加 362 万吨，交叉效应符号与命题 2 一致。H2（质量传导）：$\vartheta$ 由 0.22 提高到 0.68 时质量弹性为 0.836，与理论值 $1/(\delta-m)=0.833$ 相差 0.4\%（式 \eqref{eq:transmission}）。H3（主体选择）：进入专用市场农户的生产率中位数显著更高（1.17 对 0.97），$\varphi$ 对进入的判别力 AUC $=0.89$（命题 5）。H4（链条互补）：交叉偏导变号边界与理论直线 $\varepsilon_\Phi=1/\eta$ 的误差小于 $10^{-4}$，Leontief 极限下扩种边际收益归零（命题 7）。H5（工具替代）：储备增加使扩种的边际 CVaR 降幅由 $-0.0086$ 收缩至 $-0.0001$（命题 8 的组合互补结构）。H6（政策效率）包含两个预登记子判据：其一，目标价格政策的模拟无谓损失 4.0 亿元与解析式 $\tau^2|S'|/2$ 的预测 4.1 亿元相差 3\%（命题 9）——其中供给斜率 $|S'|$ 由统一补贴实验独立估计，两种不同政策工具给出相互一致的供给响应，构成跨工具的过度识别检验，该判据通过；其二，“定向补贴的单位财政福利增量高于统一补贴”在完整抽样下未获确认——两者分别为 0.043 与 0.046 亿元/亿元财政，差异处于蒙特卡洛噪声量级：定向补贴的总福利与产量更高但财政支出也更高（见第 9.3 小节），其相对统一补贴的优势体现在总福利、产量与收入分配维度，而非每元财政的平均效率。我们如实报告这一结果而不事后修改判据。
...
\midrule
P0 & 无大豆补贴（$\sigma\approx0$） & 0 & 0 & 1514 & 107.6 & 0.41\\
P1 & 统一面积补贴 350 元/亩 & $+21.9$ & 479 & 2055 & 80.8 & 0.41\\
P2 & 定向补贴（梯度配置） & $+22.4$ & 520 & 2093 & 78.1 & 0.41\\
P3 & 目标价格 $\tau=627$ 元/吨 & $+10.7$ & 103 & 1642 & 101.0 & 0.41\\
P4 & 质量链条包 & $+104.0$ & 515 & 1784 & 93.4 & 0.71\\
P5 & 韧性包（储备$+$多元化） & $-285.8$ & 503 & 1744 & 23.0 & 0.41\\
P6 & 组合最优（权重集中于 P4） & $+104.0$ & 515 & 1784 & 93.4 & 0.71\\
\bottomrule
\end{tabularx}
\begin{tablenotes}[flushleft]
\footnotesize
\item 注：产量为 2026--2035 窗口均值（万吨/年）；CVaR$_{5\%}$ 为最坏 5\% 年份的短缺损失（亿元）；数值为每政策 200 个种子 $\times$ 10000 农户的完整抽样结果。P5 的福利下降是以确定性成本购买尾部保险的显性保费；P6 网格下最优权重退化为质量链条包。预登记的五项排序假设中，P4 质量维度最优、P5 CVaR 最优、P3 被 P2 支配、P6$\succeq$单项包均兑现；P2$\succ$P1 以总福利与产量口径成立（参数扰动下 100\% 稳健），以每元财政口径两者基本持平（见正文）。
\end{tablenotes}
\end{threeparttable}
\end{table}

政策矩阵给出四个与理论一致的定量结论。第一，定向补贴以高约 8\% 的财政（520 对 479 亿元）换取更高的产量（2093 对 2055 万吨）、更高的总福利与更低的收入基尼（0.56 对 0.58），且总福利排序 P2$\succ$P1 在拉丁超立方抽样的参数扰动下 100\% 稳健；但每元财政的平均福利增量两者基本持平（0.043 对 0.046）——命题 6 的定向支配性在总量维度成立而幅度温和，因为现行六区补贴结构已部分内生于机会成本梯度（第 9.1 小节第二组事实），定向重新配置的剩余改进空间有限。第二，目标价格工具以最少财政换取最小的产量增量，其无谓损失与命题 9 的解析式吻合；在同等产量目标下被定向补贴支配。第三，质量链条包的福利增量（$+104$ 亿元/年）比任何单纯数量型工具高一个量级：提高 $\vartheta$ 同时改善质量选择（式 \eqref{eq:transmission}）与食用溢价实现，作用于价值维度而非数量维度——“提质”的边际回报显著高于“扩面”。第四，韧性包单独评估时平均福利为负、但使尾部损失下降 79\%（107.6 降至 23.0 亿元）：它与质量包不是相互替代，而是分别对冲不同维度的失灵（命题 3、8），组合权重应由社会的尾部风险厌恶程度决定，而非在平均福利口径下简单比较。

\subsection{底部反事实：命题 4 的定量对应}

两组反事实检验食用底部机制。其一，单纯取消大豆生产者补贴而保持玉米补贴不变（P0，$\sigma\approx0$），产量收敛于约 1500--1700 万吨区间（2026--2035 窗口均值 1514 万吨，转型完成后的稳态约 1692 万吨），远高于零，也显著高于 2015 年低点：食用溢价在产量收缩时内生上升至约 900--1200 元/吨，恰好覆盖边际产区含玉米机会成本的边际成本。其二，复现 2015 年式的双重极端（大豆零补贴且玉米临储使 $\pi^c$ 抬高约 250 元/亩，即 $\sigma<0$），稳态产量降至约 1342 万吨——与 2015 年实际低点（1179 万吨，按今日单产折算约 1320 万吨）高度接近。

两组数字的差值本身即是一个政策发现：2015 年的深度收缩是负补贴差与玉米临储的叠加产物，而非“无补贴即归零”的证据；在玉米临储已经退出的今日，即便完全取消大豆生产者补贴，市场化底部也在 1700 万吨上下（稳态口径）。因此，约 500 亿元/年的补贴差支出实际购买的是市场化底部与社会最优之间约 400--600 万吨的缺口及其安全与质量收益——政策辩论的正确对象是这一边际量的成本收益，而不是产业的存续本身。
...
定量校准赋予上述机制以数量内容。以分省成本收益面板构造的含玉米机会成本的供给曲线，使模型以不到1\%的偏差复现观测产量；六项理论命题在合成代表性农户仿真中，五项完全通过结构检验，政策效率命题的无谓损失判据通过跨工具过度识别检验（每元财政效率在两种补贴工具间基本持平）；黑龙江"剩余差$-182$元/亩对补贴差$+248$元/亩"的边际无差异、补贴强度与玉米机会成本的空间同构，为玉米—大豆联动提供了三重实证交叉验证。政策矩阵表明：面积配置的有效杠杆是大豆—玉米补贴差而非大豆补贴的绝对水平；定向补贴在总福利、产量与分配上优于统一补贴；质量链条包的福利回报比数量型工具高一个量级；韧性包以显性保费购买尾部保险。食用需求的内生溢价则保证：即便补贴差归零，国产大豆也存在约1500--1700万吨的市场化底部——现行约500亿元/年的支出购买的是底部与社会最优之间400--600万吨的边际缺口，这才是政策评估的正确对象。
