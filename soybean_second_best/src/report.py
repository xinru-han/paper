...
def run(cfg=None, meta=None):
    ...
    lines = [
        ...
        f"- **理论命题检验 {n_pass}/6 通过**（T6 之 DWL 判据通过、每元财政效率判据两补贴基本持平）；福利排序：组合最优 P6=质量链条包 P4 ≻ 定向补贴 P2 ≻ 统一补贴 P1 ≻ 目标价格 P3 ≻ 无干预 P0（韧性包 P5 平均福利为负但 CVaR 最优）。\n",
        "## 2 数据与校准\n",
        "- 投入产出: 《2023年全国投入产出表》211 产品部门（b01.xls), 按 src/io_aggregate.py 聚合 9 部门, 大豆从\"其他农产品\"以产值锚拆分（国产 971 亿 + 进口 3877 亿元）;",
        "- 成本收益: 《全国农产品成本收益资料汇编》历年分省面板（2006–2024, 24 省, 大豆+玉米, "
        "src/extract_cost_panel.py → data/cost_benefit_panel.csv）; 6 区参数为 2022–24 三年面积加权均值 (src/build_regions.py);",
        "- 玉米—大豆联动: 社会边际成本 MC_豆 = (大豆生产成本 + 放弃玉米的剩余)/单产; 农户选择差 "
        "ΔV = (豆剩余+豆补贴) − (玉米剩余+玉米补贴), 政策杠杆是**补贴差**而非大豆补贴绝对水平"
        "（实证锚: 黑龙江 2024 豆−玉米剩余差 −182 元/亩 ≈ −补贴差 248 元/亩 → 边际无差异）;",
        "- 供给曲线校准至 MC(2090) = 观测边际激励 6123 元/吨（食用拍卖价 4298 + 补贴差当量 248/0.1359）;",
        "- 食用底部: 国产豆食用反需求过观测点 (2065 万吨, 溢价 627 元/吨), 弹性 ε_f=0.5; "
        "历史底部 Y_min = 1100 万吨（NBS 历史产量序列, 2015 年低点 1179 万吨）;",
        f"- 结构参数: cost_scale={d.get('cost_scale')}, ℓ={d.get('ell_qty')} 亿元/万吨², β={d.get('beta_security')}（揭示性校准, 见 §M1）, "
        f"Logit τ=600（校准至观测面积年际弹性）, SMM 稳态修正 δ={d.get('abm_smm_delta')}; anchor/struct 标注见 data/*.csv 的 calib_flag 列;",
        "- **ABM 口径说明**: M4/M5 的农户为**合成代表性农户**（按 6 区实测成本收益+面积权重生成的异质性个体）, "
        "不是入户调查微观样本; 结论的微观基础是分省成本收益面板与区域面积份额, 个体异质性(φ_i, 地块规模)为参数化设定。\n",
        ...
        "**预登记假设兑现**: P2≻P1 总福利/产量口径 ✓（M7 扰动下 100% 稳健; 每元财政效率口径完整抽样下两者基本持平, "
        "0.043 vs 0.046, 详见 T_M6 之 T6, 如实报告不事后改判据）; P4 质量维度最优 ✓; P5 CVaR 维度最优 ✓; "
        "P3 被 P2 支配 ✓; P6≻单项包 ✓。\n",
        "## 5 命题检验记分卡\n",
